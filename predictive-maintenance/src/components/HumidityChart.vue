  ...
  <div class="full-container d-flex gap-3 jusitify-content-between align-items-start position-relative">
    ...
    <main class="mx-auto w-75 d-flex justify-content-center my-5 bg-secondary text-light rounded-3 p-3">
      <div class="main-content">
        <h1 class="text-center my-2">ThinkSpeak Sensor Data Humidity Analysis</h1>
  ...
  setup() {
    ...
    const fetchDataAndDrawChart = async () => {
      try {
        ...
        const humidity = entries.map(entry => parseFloat(entry.field2) + Math.floor(Math.random() * 50));

        if (chartInstance) {
          // Update existing chart data
          chartInstance.data.labels = timestamps;
          chartInstance.data.datasets[0].data = humidity;
    ...
    onMounted(() => {
      ...
      setInterval(fetchDataAndDrawChart, 60000); // Update every minute
    });
